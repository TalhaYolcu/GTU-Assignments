...
public class ChestGameStarter : MonoBehaviour{
    ...
    void Update(){
        ...
        if(Input.touchCount>0) {
            if((Input.touchCount>0 && Input.GetTouch(0).phase==TouchPhase.Began && touchneeded) || Input.GetMouseButtonDown(0) && touchneeded) {
                ...
                if(Physics.Raycast(ray,out _hitinfo)) {
                    ...
                    //if(col!=null) {
                        isSelected=true;
                        //_hitinfo=Physics2D.GetRayIntersection(ray);
                        selectedObject=_hitinfo.transform.gameObject;
                        OnSelected();
                    }
                //} 
    ...
    IEnumerator Finish() {
        text.text="You won,Game is finished, returning back to the previous scene in 2 seconds";
