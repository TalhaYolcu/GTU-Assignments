...
namespace Talha_Abstract_Hex {
	...
	template<template<typename ...>class T>
	HexAdapter<T>::HexAdapter(int size,int user) : AbstractHex(size,user){
		int i;
		hexCells.resize(get_size()*get_size());
		for(i=0;i<get_size();i++) {
			hexCells.at(i).resize(get_size());
	...
	template<template<typename ...>class T>		
	void HexAdapter<T>::playGame()						// ask board_size, user vs user or user vs computer
	{
	int i;
		...
		template<template<typename ...>class T>	
	int HexAdapter<T>::writeToFile(string &input2) {				//LOAD A GAME FROM FILE
		int ret=-1;
		ifstream wr2;
		wr2.open(input2);
		...
		template<template<typename ...>class T>	
	int HexAdapter<T>::ready_to_play() {
