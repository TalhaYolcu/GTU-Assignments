...
#include "named_implementation.c"
...
int main(int argc, char const *argv[]) {
    ...
    char name_semaphore[MAX_BLKSIZE];

    memset(inputfilepath,0,MAX_BLKSIZE*sizeof(char));
    memset(name_semaphore,0,MAX_BLKSIZE*sizeof(char));
    ...
    for(i=1;i<argc;i++) {
        if(argv[i][0]=='-' && argv[i][1]=='i' && i!=(argc-1)) {
            ...
            strncpy(name_semaphore,argv[i+1],strlen(argv[i+1]));
        }                    
    }     
    ...
    int memory_fd=0;
    //create shared memory and open semaphores
    if(create_shared_memory(&memory_fd,sizeof(shared_memory),name_semaphore)==-1) {
    ...
    int read_byte=0;
    int row_count=0;
    char read_ingredients[TOTAL_INGREDIENTS-ENDLESS_SUPPLY];    
    memset(read_ingredients,0,(TOTAL_INGREDIENTS-ENDLESS_SUPPLY)*sizeof(char));
    ...
    if(destroy_semaphores(name_semaphore)==-1) {
